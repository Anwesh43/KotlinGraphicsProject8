...
class ArcLineExtendRotView(ctx : Context) : View(ctx) {
    ...
    data class ALERNode(var i : Int = 0, val state : State = State()) {

        private var next : ALERNode? = null
        private var prev : ALERNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawALERNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : ALERNode {
            var curr : ALERNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ArcLineExtendRot(var i : Int) {

        private var curr : ALERNode = ALERNode(0)
