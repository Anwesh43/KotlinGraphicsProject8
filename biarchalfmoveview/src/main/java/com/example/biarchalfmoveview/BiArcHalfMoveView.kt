package com.example.biarchalfmoveview
...
class BiArcHalfMoveView(ctx : Context) : View(ctx) {
    ...
    data class BAHMNode(var i : Int = 0, val state : State = State()) {

        private var prev : BAHMNode? = null
        private var next : BAHMNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBAHMNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BAHMNode {
            var curr : BAHMNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BiArcHalfMove(var i : Int) {

        private var curr : BAHMNode = BAHMNode(0)
